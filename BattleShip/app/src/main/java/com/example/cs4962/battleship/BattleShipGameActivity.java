...
public class BattleShipGameActivity extends Activity
{
    ...
    private void loadData(String gameName)
    {
        ...
        switch (status)
        {
            case DEPOLOYMENT_PLAYER1:
                FragmentTransaction trans = getFragmentManager().beginTransaction();
                DeploymentFragment fragment = DeploymentFragment.newInstance(gameName,manager.getPlayer1Board(gameName).getPlacedShips().size());
                fragment.setOnPassButtonClick(onPassButtonClickListener());
                if(tablet)
                {
              ...
              //  trans.replace(masterFrameLayout.getId(), fragment);
                ...
                trans.commit();
                break;
            case DEPOLOYMENT_PLAYER2:
                trans = getFragmentManager().beginTransaction();
                fragment = DeploymentFragment.newInstance(gameName, manager.getPlayer2Board(gameName).getPlacedShips().size());
                if(tablet)
                {
                    secondFrameLayout.removeAllViews();
                    trans.add(secondFrameLayout.getId(),fragment);
                ...
                trans.commit();
                manager.setGameMode(gameName, status);
                fragment.setOnDoneButtonClick(onDoneButtonClickListener());
                break;
            case PLAYER1_TURN:
    ...
    public BattleShipManager.OnNewGameListener onNewGameListener()
    {
        BattleShipManager.OnNewGameListener listener = new BattleShipManager.OnNewGameListener()
        {
            ...
            {
                FragmentTransaction trans = getFragmentManager().beginTransaction();
                DeploymentFragment fragment = DeploymentFragment.newInstance(name,manager.getPlayer1Board(name).getPlacedShips().size());
                fragment.setOnPassButtonClick(onPassButtonClickListener());
                if(tablet)
                {
                    secondFrameLayout.removeAllViews();
                ...
                manager.setGameMode(name, GameStatus.DEPOLOYMENT_PLAYER1);
                trans.commit();
            }
        };
        return listener;
    ...
    public DeploymentFragment.OnPassButtonClickListener onPassButtonClickListener()
    {
        DeploymentFragment.OnPassButtonClickListener listener = new DeploymentFragment.OnPassButtonClickListener()
        {
            @Override
            public void onPassButtonClick(String name)
            {
                    FragmentTransaction trans = getFragmentManager().beginTransaction();
                    DeploymentFragment fragment = DeploymentFragment.newInstance(name,manager.getPlayer2Board(name).getPlacedShips().size());
                if(tablet)
                {
                    secondFrameLayout.removeAllViews();
                    trans.add(secondFrameLayout.getId(),fragment);
                ...
                else
                {
                   ...
                   // masterFrameLayout.removeAllViews();
                    //trans.replace(masterFrameLayout.getId(), fragment);
                    trans.commit();
                    manager.setGameMode(name, GameStatus.DEPOLOYMENT_PLAYER2);
                    fragment.setOnDoneButtonClick(onDoneButtonClickListener());
    ...
    public DeploymentFragment.OnDoneButtonClickListener onDoneButtonClickListener()
    {
        DeploymentFragment.OnDoneButtonClickListener  listener = new DeploymentFragment.OnDoneButtonClickListener()
        {

            @Override
            public void onDoneButtonClick(String name)
            {
                int turn = manager.getPlayer1Board(name).getMisslesFired() + manager.getPlayer2Board(name).getMisslesFired();
                FragmentTransaction trans = getFragmentManager().beginTransaction();
                GameFragment fragment = GameFragment.newInstance(name,turn);
                if(tablet)
                {
                    secondFrameLayout.removeAllViews();
                    trans.add(secondFrameLayout.getId(),fragment);
                ...
                //trans.replace(masterFrameLayout.getId(), fragment);
                trans.commit();
                manager.setGameMode(name, GameStatus.PLAYER1_TURN);
            }
        };
